...
export const TrendsDisplay = () => {
    ...
    const fetchHistory = async () => {
        const { data } = await supabase
            ...
            .order('created_at', { ascending: false }) // Get latest first for processing limits
            .limit(100);
    ...
    const stats = useMemo(() => {
        return matches.reduce((acc, curr) => {
            if (curr.winner === 'meron') acc.meron++;
            else if (curr.winner === 'wala') acc.wala++;
            else if (curr.winner === 'draw') acc.draw++;
            return acc;
        }, { meron: 0, wala: 0, draw: 0, total: matches.length });
    }, [matches]);
                ...
                {matches.map((match) => (
                    <div key={match.id} className="w-6 h-6 flex items-center justify-center">
                        <div className={clsx(
                            ...
                            {match.winner === 'meron' ? 'M' : match.winner === 'wala' ? 'W' : 'D'}
                        </div>
                    </div>
                ))}
    ...
    const bigRoadColumns = useMemo(() => {
        ...
        matches.forEach((match) => {
            // Skip if no winner (shouldn't happen with filter)
            if (!match.winner) return;

            // Simple logic: If same winner (or first item), add to current column
            // Else start new column
            // Note: Standard Big Road is complex with "Dragon Tails" (L-shapes).
            // We will implement a "Simple Big Road" where it just stacks vertically and breaks to new col on change.
                                        ...
                                        {col.map((match, j) => (
                                            <div key={`${match.id}-${j}`} className="w-6 h-6 flex items-center justify-center">
                                                <div className={clsx(
                                                    ...
                                                    {/* Big Road usually just hollow circles, showing dot or line for variance, simple circle here */}
                                                </div>
                                            </div>
                                        ))}
                                    </div>
